...
// problem
if (!$id) {
	echo "<p>Select a <a href=\"index.php\">variant</a> to edit.</p>";
// new variant
} else if ($id=="new") {
	$insert=mysql_query("INSERT INTO variant (name) VALUES ('$name')") or die("Problem updating " . mysql_error());
	echo "$name added.";
// edit variant
} else {
	$update=mysql_query("UPDATE variant SET name='$name' WHERE id='$id'") or die("Problem updating " . mysql_error());
	echo "$name updated.";
}
?>
